...
probes the physical limits of HPC set by collecting, refocusing, and recording
the superposition of the incident beam and scattered wave.

\begin{figure}
  \centering
  \includegraphics[angle=-90,origin=c,width=0.85\textwidth]{debye_wolf_schematic_2}
...
of interests. In the proceeding sections, we will perform the following calculations
...
\end{enumerate}
The computations at each plane require a geometry to define the fields
on; Fig. \ref{fig:debye_schematic}(b) demonstrates the geometries for
the entrance pupil of the objective, the tube lens, and the camera's
...
During this transit, the fields refract through the water-glass interface as they
exit the sample volume. Fig.~\ref{fig:waterglass} illustrates the Fresnel transmission
coefficients of fields refracting through the interface at angle $\theta$.
As our fields are dominated by low-angle scattering, we safely neglect the
attenuation and refraction through this interface. Since we utilize an
oil-immersion objective whose oil refractive index, $n_{\text{oil}}$,
nearly matches glass, we also neglect the secondary refraction treated by
Ovryn and Izen \cite{izen00}.
...
\begin{equation}
  \label{eq:entrance_pupil}
    \vec{E}_s(\theta, \phi)|_{\text{scatterer plane}} \approx \vec{E}_s(\theta, \phi)|_{\text{focal plane}} e^{-i\frac{2\pi}{\lambda}z\cos{\theta} }.
  \end{equation}
...
and $n'$ is the refractive index of air the outside the camera.
...
surface to the scattered field, $\vec{E}_{s,\text{img}}$ present in the focal plane of that lens
\begin{equation}
  \vec{E}_{s,\text{img}}(x', y') = \frac{i k'}{2 \pi} \iint_{\Omega_{\text{img}}} \vec{E}_s'(s'_x, s'_y) e^{-ik'(s'_xx'+s'_yy')}\frac{ds'_xds'_y}{\cos{\theta'}}
  \label{eq:debyewolf}
\end{equation}
...
that $s_x^2+s_y^2 \le \text{NA}/n_{\text{oil}}$. Note that $\cos \, \theta' = \sqrt{ 1 - (s_x^{\prime 2} + s_y^{\prime 2} )}$.
...
\item The Debye-Wolf transformation takes $\vec{E}'$, a strength factor with units
  ...
  returns the electric field  in cartesian coordinates present in plane \num{4}.
\item The transformation appears to be distance agnostic; the distance
  ...
  such that the distance from plane \num{3} to plane \num{4} must be exactly
  $M \, f$ apart.
\end{enumerate}
...
The propagation of the incident field through the optical system is more easily explained
than the scattered field. The incident field travels parallel to the optical
axis towards the entrance pupil of the objective. The objective
and tube lens then laterally magnify the incident field and induce a $\pi$-shift in phase.
...
The image in the camera plane is then given as
\begin{equation}
  I(x', y') = \preint \left | E'_{\text{inc}}(x',y') + E'_s(x', y') \right |^2.
\end{equation}
...
holograms with the vector-based theory over a range \% Insert range.%%FIXME.
We will then fit each synthetic hologram to the scalar-based theory as outlined
