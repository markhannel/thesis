...
the scalar theory in favor of the vector theory. Over the majority of the
range, the error in diameter and axial displacement are bounded by $\SI{1}{\nm}$
while the error in refractive index is bounded by $\SI{0.001}{}$; in all three
parameters, the maximum error is less than the reported resolution of
the technique \cite{krishnatreya14}.
...
demagnification. The additional computational complexity greatly increases
the time spent analyzing holograms.

The image in the focal plane can be modified to incorporate certain
optical limitations, such as low-NA imaging \cite{saglimbeni16}.
Our formalism easily accommodates phase-aberrations including spherical aberrations
caused by the optical train and can be used to model holographic images
produced with non-ideal optics.
