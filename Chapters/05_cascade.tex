...
from converging to reasonable results. Previous works
have demonstrated multiple heuristic algorithms that achieve
...
to dense or heterogeneous samples. Furthermore, the computational
complexity of such algorithms currently prohibit real-time applications.
...
\begin{figure}[b!]
  ...
  \caption{ Depicting the problem of feature detection and localization.
    (a) each extended holographic feature in the recorded image (left)
    must be localized by a bounding box (right).
    (b) recapitulating the same task as a mathematical problem. \%\%FIXME: The bounding box values are wrong.. sorry David!} 
  \label{fig:detection}
\end{figure}
...
holographic feature in a video frame, return a set of coordinates
identifying a bounding box centered around the feature. As depicted
in figure~\ref{fig:detection}(a), features may vary in
contrast and extent. Additionally, features may overlap or be occluded by
the edge of the field of view.

Figure~\ref{fig:detection}(b) conveys the computational difficulty
of automating this task. A two-dimensional array stores the intensity values
...
Fig.~\ref{fig:detection}(a) represents a
...
\begin{figure}[b!]
  ...
  \caption{ A random selection of cropped and centered
  holographic features used in training.} 
...
We utilize Eq.~\eqref{eq:gen_norm} - \eqref{eq:bn} to generate training 
...
deviation of five percent. Fig.~\ref{fig:rogue_gallery} depicts
\num{16} randomly selected, cropped, and centered holographic features
used in training.
...
\begin{figure}[!t]
  \centering
  \includegraphics[width=\columnwidth]{cascade_02} % XXX CHECK that this
