...
\% FIXME: the next two mini-paragraphs should be either nixed or incorporated
into the two-paragraph introduction.
...
TPM particles require no specialized equipment or inert environment.
However, there are many parameters in the synthesis that can affect the average size and refractive
index for a synthesized population.
% FIXME: Isn't this sentence a repeat of the end of the first section?
...
%% FIXME: This sentence feels a bit late. The following sentence is a semi-repeat
% of before but adds the specific things we will be gauging.
...
the average of the each oval's minor and major axis provides an estimate for the
...
\begin{figure}
    ...
    \caption{A scatterplot of the measured refractive index of \num{12} solutions with differing
      volume fractions of TPM; each measurement plotted as a filled blue circle.
...
with a \SI{95}{\percent} confidence that it lies in the range $\left ( 1.499, 1.513 \right )$.
...
% techniques to HPC.
% Comment on how you will not be using comparative analysis with SEM in the following
% sections but that SEM images were taken of each sample for reassurance.
...
% That would be visible in our HPC results.
...
time spent thermally coupled to a heat bath greatly accelerates polymerization. % OR is it necessary for polymerization?
As observed in Fig.~\ref{fig:orthogonal_vs_hpc}(a), the polymerization of
...
refractive index serves as a proxy for the degree of polymerization of a sample.
...
Each sample is added to \SI{1}{\ml} of room temperature DI water, which effectively
